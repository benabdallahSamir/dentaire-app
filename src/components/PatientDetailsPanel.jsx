...
function PatientDetailsPanel({ isOpen, onClose, patient }) {
  ...
  useEffect(() => {
    if (isOpen && patient) {
      // Reset stale data and tab before fetching for the new patient
      setSessions([]);
      setPackages([]);
    ...
    if (!isOpen) {
      // Clear data when panel closes so it doesn't briefly flash old data next open
      setSessions([]);
      setPackages([]);
    }
  }, [isOpen, patient?.id]);
  ...
  const fetchPatientHistory = async () => {
    ...
    try {
      ...
      console.log('[PatientDetailsPanel] sessionsRes:', sessionsRes);
      console.log('[PatientDetailsPanel] packagesRes:', packagesRes);
  ...
  return (
    <>
      <div 
        className={`fixed inset-y-0 right-0 w-[500px] bg-white shadow-2xl z-50 transform transition-transform duration-500 ease-out border-l border-neutral-100 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header Section */}
        <div className="p-8 border-b border-neutral-100 bg-neutral-50/50">
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h2 className="text-2xl font-black text-neutral-900 capitalize leading-tight">
                  {patient.name}
                </h2>
          ...
          {/* Tab Navigation */}
          <div className="flex p-1 bg-neutral-100/50 rounded-2xl">
            {tabs.map((tab) => (
              <button
                key={tab.id}
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
                    ...
                    <p className="text-sm font-bold text-neutral-800">{patient.phone || 'Not provided'}</p>
                  </div>
                  <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
                    <div className="flex items-center gap-3 text-neutral-400 mb-2">
                       <Calendar size={14} />
                    ...
                    <p className="text-sm font-bold text-neutral-800">{patient.dob || '--/--/----'}</p>
                  </div>
                  <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
                    <div className="flex items-center gap-3 text-neutral-400 mb-2">
                       <Activity size={14} />
                    ...
                    <p className="text-sm font-bold text-neutral-800">{patient.gender || 'Not specified'}</p>
                  </div>
                  <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
                    <div className="flex items-center gap-3 text-neutral-400 mb-2">
                       <MapPin size={14} />
                    ...
                    <p className="text-sm font-bold text-neutral-800 truncate" title={patient.address}>
          ...
          {activeTab === 'sessions' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              {loading ? (
                ...
                sessions.map((s) => (
                  <div key={s.id} className="group p-5 bg-white rounded-3xl border border-neutral-100 hover:border-blue-500/50 transition-all">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${s.package_id ? 'text-emerald-600 bg-emerald-50' : 'text-blue-600 bg-blue-50'}`}>
                            {s.package_id ? 'Sub Session' : 'Treatment'}
                          </span>
                        </div>
                        <h5 className="font-bold text-neutral-800">
                          {s.package_id ? 'Sub Session' : 'Session'} on {s.date}
                        </h5>
                      </div>
                    ...
                    {s.note && (
                      <div className="p-3 bg-neutral-50 rounded-xl">
                        <p className="text-xs text-neutral-500 italic">"{s.note}"</p>
                      </div>
                    )}
                  </div>
                ))
          ...
          {activeTab === 'packages' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              {loading ? (
                ...
                packages.map((pkg) => (
                  <div key={pkg.id} className="p-5 bg-white rounded-3xl border border-neutral-100 hover:border-emerald-500/50 transition-all">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h5 className="font-black text-neutral-900 uppercase tracking-tight">{pkg.name}</h5>
